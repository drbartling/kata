...
{
    CBF_BUFFER_T buffer = CBF_BufferNew(int, 42);
    TEST_ASSERT_NOT_NULL(buffer);
    CBF_BufferDelete(buffer);
}

void test_BufferNew_Should_CreatePointerToBuffer(void)
{
    CBF_BUFFER_T buffer = CBF_BufferNew(long, 2);
...
{
    CBF_BUFFER_T buffer = CBF_BufferNew(int, 42);
    TEST_ASSERT_EQUAL_PTR(buffer->buffer, buffer->write);
    CBF_BufferDelete(buffer);
}

void test_BufferNew_should_intializeReadToBufferStart(void)
{
    CBF_BUFFER_T buffer = CBF_BufferNew(float, 29);
    TEST_ASSERT_EQUAL_PTR(buffer->buffer, buffer->read);
    CBF_BufferDelete(buffer);
}
...
void test_ElementRead_and_Write_workTogether_to_readAndWriteData(void)
{
...
int main(void)
{
    ...
    RUN_TEST(test_ElementRead_and_Write_workTogether_to_readAndWriteData);
